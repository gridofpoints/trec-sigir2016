%% threeFactorAnalysis
% 
% Performs a three factors analysis on the considered experimental
% collections.

%% Synopsis
%
%   [] = threeFactorAnalysis(trackID, startMeasure, endMeasure)
%  
%
% *Parameters*
%
...
function [] = threeFactorAnalysis(trackID, startMeasure, endMeasure)
    ...
    % turn on logging
    delete(EXPERIMENT.pattern.logFile.analysis(EXPERIMENT.analysis.threeF.name, trackID));
    diary(EXPERIMENT.pattern.logFile.analysis(EXPERIMENT.analysis.threeF.name, trackID));

    % start of overall computations
    startComputation = tic;

    fprintf('\n\n######## Computing three factor analyses on collection %s (%s) ########\n\n', EXPERIMENT.(trackID).name, EXPERIMENT.label.paper);
    ...
    fprintf('  - analysis type: %s\n', EXPERIMENT.analysis.threeF.description);
    fprintf('  - slice \n');
    ...
    for m = 1:startMeasure:endMeasure
        ...
        for lug = 1:EXPERIMENT.taxonomy.lug.number
            ...
            anovaStatsID = EXPERIMENT.pattern.identifier.anovaStats(EXPERIMENT.analysis.threeF.id, lugID, measureID);
            anovaSoAID = EXPERIMENT.pattern.identifier.anovaSoA(EXPERIMENT.analysis.threeF.id, lugID, measureID);
            anovaSfericityID = EXPERIMENT.pattern.identifier.anovaSfericity(EXPERIMENT.analysis.threeF.id, lugID, measureID);            
            
            
            evalf(EXPERIMENT.analysis.threeF.command, ...
                {measureID, 'lugID'}, ...
                {anovaTableID, anovaStatsID, anovaSoAID, anovaSfericityID});
            
            fprintf('    * saving analyses\n');
            sersave(EXPERIMENT.pattern.file.anova(trackID, EXPERIMENT.analysis.threeF.id, lugID, measureID), ...
                anovaTableID(:), anovaStatsID(:), anovaSoAID(:), anovaSfericityID(:))
    ...
    fprintf('\n\n######## Total elapsed time for computing three factor analyses on collection %s (%s): %s ########\n\n', ...
